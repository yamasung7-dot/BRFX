 ...
 * Full Environment System v1.0.0
...
Plugin.register('brfx', {
    ...
    onload() {
        ...
        plugin.refresh = function() {
            ...
            if (typeof Preview !== 'undefined' && Preview.selected && typeof Preview.selected.render === 'function') { try { Preview.selected.render(); } catch (error) {} }
        };
        ...
        plugin.getModelCenter = function() {
            ...
            if (typeof Outliner !== 'undefined' && Array.isArray(Outliner.elements)) Outliner.elements.forEach(element => {
                ...
                try { element.mesh.updateMatrixWorld(true); const b = new THREE.Box3().setFromObject(element.mesh); if (!b.isEmpty()) { box.union(b); found = true; } } catch (error) {}
            });
        ...
        plugin.getBillboardPosition = function(b) {
            ...
            if (typeof b.getWorldCenter === 'function') { try { const p = b.getWorldCenter(); if (p) return p.clone(); } catch (error) {} }
            return null;
        };
        ...
        plugin.addEnvironmentLights = function() {
            ...
            main.name = 'BRFX_Environment_Point_Light'; main.castShadow = false; main.position.copy(c.clone().add(new THREE.Vector3(0,8,6))); targetScene.add(main); plugin.sceneLights.push(main);
            ...
            if (plugin.settings.indirect) {
                ...
                hemi.name = 'BRFX_Indirect_Ambient_Light'; hemi.position.copy(c); targetScene.add(hemi); plugin.ambientLight = hemi;
            }
        ...
        plugin.removeSky = function() {
            if (plugin.skyDome) { const sky = plugin.skyDome; try { if (sky.parent) sky.parent.remove(sky); } catch (error) {} try { if (sky.geometry) sky.geometry.dispose(); if (sky.material) { if (sky.material.map) sky.material.map.dispose(); sky.material.dispose(); } } catch (error) {} plugin.skyDome = null; plugin.skyTexture = null; }
        };
        plugin.createProceduralSky = function() {
            const targetScene = plugin.getScene(); if (!targetScene || typeof THREE === 'undefined') return null;
            plugin.removeSky();
            const top = plugin.normalizeColor(plugin.settings.skyTop, '#4f82c4'), horizon = plugin.normalizeColor(plugin.settings.skyHorizon, '#ffd6a0'), bottom = plugin.normalizeColor(plugin.settings.skyBottom, '#6f5748');
            const tw = Math.max(0, Math.min(100, Number(plugin.settings.skyTopWeight) || 0)), hw = Math.max(0, Math.min(100, Number(plugin.settings.skyHorizonWeight) || 0)), bw = Math.max(0, Math.min(100, Number(plugin.settings.skyBottomWeight) || 0)), total = Math.max(.001, tw + hw + bw);
            const topStop = (bw + tw) / total, horizonStop = bw / total;
            ...
            const sky = new THREE.Mesh(geometry, material); sky.name='BRFX_Procedural_Sky_Dome'; sky.frustumCulled=false; sky.renderOrder=-100000; sky.rotation.y=THREE.MathUtils.degToRad(Number(plugin.settings.skyRotation)||0); targetScene.add(sky); plugin.skyDome=sky; return sky;
        };
        plugin.loadImageSky = function(url) {
            const targetScene = plugin.getScene(); if (!targetScene || typeof THREE === 'undefined' || !url) return null;
            plugin.removeSky();
            try {
                ...
                loader.load(url, texture => {
                    ...
                    const sky = new THREE.Mesh(geometry, material); sky.name='BRFX_Image_Skybox'; sky.frustumCulled=false; sky.renderOrder=-100000; sky.rotation.y=THREE.MathUtils.degToRad(Number(plugin.settings.skyRotation)||0); targetScene.add(sky); plugin.skyDome=sky; plugin.skyTexture=texture; plugin.refresh();
                }, undefined, () => Blockbench.showQuickMessage('BRFX: Could not load image skybox URL',3000));
                return true;
            } catch (error) { return null; }
        };
        plugin.applyAtmosphere = function() {
            const targetScene = plugin.getScene(); if (!targetScene) return;
        ...
        plugin.applySky = function() {
            ...
            if (plugin.settings.skyMode === 'image' && plugin.settings.skyImageUrl) plugin.loadImageSky(plugin.settings.skyImageUrl); else plugin.createProceduralSky();
        };
        plugin.applyAll = function(showMessage=true) {
            ...
            plugin.settings.skyTop = plugin.normalizeColor(plugin.settings.skyTop,'#4f82c4'); plugin.settings.skyHorizon = plugin.normalizeColor(plugin.settings.skyHorizon,'#ffd6a0'); plugin.settings.skyBottom = plugin.normalizeColor(plugin.settings.skyBottom,'#6f5748'); plugin.settings.fogColor = plugin.normalizeColor(plugin.settings.fogColor,'#b8a58f');
            if (plugin.settings.lightType === 'environment') plugin.addEnvironmentLights(); else {
                plugin.removeSceneLights();
                ...
                if (typeof Sun !== 'undefined' && Sun) { try { Sun.color.set(plugin.settings.lightColor); Sun.intensity=Math.max(0,Number(plugin.settings.lightIntensity)||0); } catch (error) {} }
            }
            plugin.applySky(); plugin.applyAtmosphere(); plugin.refresh();
        ...
        plugin.openSettings = function() {
            ...
            plugin.customDialog = new Dialog({id:'brfx_v1_settings', title:'BRFX 1.0 — Environment Settings', width:560, form:{
                ...
                plugin.settings.lightSide = Number(form.lightSide)||0; plugin.settings.skyRotation=Number(form.skyRotation)||0; plugin.settings.exposure=Number(form.exposure)||1; plugin.settings.contrast=Number(form.contrast)||1;
                plugin.applyAll();
            }});
            plugin.customDialog.show();
        };

        plugin.addAction = function(id,name,icon,click) { const action = new Action(id,{name,icon,click}); if (typeof MenuBar !== 'undefined' && MenuBar.menus && MenuBar.menus.tools) MenuBar.menus.tools.addAction(action); plugin._actions.push(action); return action; };
        plugin._actions=[];
